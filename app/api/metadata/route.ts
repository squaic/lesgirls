...
async function safeUrl(raw: string) {
  ...
  try {
    ...
    throw new Error("Ce site est introuvable.");
  }
  ...
  for (let i = 0; i <= MAX_REDIRECTS; i++) {
    ...
    try {
      res = await fetch(url, {
        ...
        headers: {
          "user-agent": "Mozilla/5.0 (compatible; LesGirlzBot/1.0)",
    ...
    if (!res.ok) {
      throw new Error("Ce site ne répond pas.");
    }
    ...
    if (declared > MAX_BYTES) {
      throw new Error("Cette page est trop volumineuse.");
    }

    const reader = res.body?.getReader();
    if (!reader) {
      throw new Error("Contenu inaccessible.");
    }
...
export async function POST(req: Request) {
  try {
    ...
    const initial = await safeUrl(String(body.url || ""));
    ...
    return NextResponse.json({
      ...
      sourceName: (meta(html, "og:site_name") || url.hostname.replace(/^www\./, "")).slice(
        0,
        120,
      ),
    ...
    return NextResponse.json(
      { error: e instanceof Error ? e.message : "Impossible d’analyser ce lien." },
      { status: 400 },
    );
  }
}
