...
export default async function Profile() {
  ...
  return <main className="shell min-h-dvh"><div className="min-h-[calc(100dvh-62px)] px-5 py-6"><header className="flex items-center justify-between"><Link href="/" className="icon-button" aria-label="Retour"><ArrowLeftIcon /></Link><Brand small /><span className="w-10" /></header><section className="pt-12 text-center"><div className="mx-auto flex h-20 w-20 items-center justify-center rounded-full border border-[var(--line)] bg-[var(--cream)] serif text-3xl">{firstName[0].toUpperCase()}</div><h1 className="serif mt-5 text-[30px] font-normal">{firstName}</h1><p className="mt-1 text-xs text-[var(--muted)]">{user.email}</p></section>{membership && <section className="mt-10 border-y border-[var(--line)] py-6"><p className="text-[9px] uppercase tracking-[.2em] text-[var(--muted)]">Votre groupe</p><h2 className="serif mt-2 text-[26px]">{membership.groups.name}</h2><p className="mt-1 text-xs text-[var(--muted)]">{membership.role === "owner" ? "Propriétaire" : "Membre"}</p><div className="mt-5 bg-[var(--cream)] p-4"><p className="mb-2 text-[10px] uppercase tracking-[.12em]">Invitation privée</p><p className="break-all text-xs leading-5 text-[var(--muted)]">{invite || `/join/${membership.groups.invite_code}`}</p></div></section>}<div className="mt-9 text-center"><SignOut /></div></div><BottomNav /></main>;
}
